...
run_suite() {
  local suite=$1
  local output_file=".ci-evidence/e2e-output.txt"
  echo "Running $suite" | tee .ci-evidence/e2e-suite.txt
  (cd "$(dirname "$suite")" && "./$(basename "$suite")") 2>&1 | tee "$output_file"
  local rc=${PIPESTATUS[0]}
  if [[ $rc -ne 0 ]]; then
    echo "FAIL: $suite exited with status $rc" >&2
    exit 1
  fi
}

# 1 — Try the fresh-deploy suite in-place
for candidate in \
...
# 2 — Extract fresh-deploy suite from a release archive
if [[ -n "$release_path" && -f "$release_path" ]]; then
  mkdir -p .ci-clean-room
