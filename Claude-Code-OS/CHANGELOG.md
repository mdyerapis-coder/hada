...
A chat-reliability release — the agent chat now keeps its memory across turns and
always hands back the complete result.

### Fixed — the chat remembers itself

- **Session continuity** — typed conversations now carry their session from one
  turn to the next, so the agent remembers what it just did. Build something,
  then say "save that to my Desktop" and it knows exactly what you mean.
  (Previously every message started a blank-slate session, so a follow-up like
  "save this" came back as "what's 'this'?".)

### New — always the full result, never a stub

- **Full-output contract** — when you ask the agent to produce code, HTML, or a
  document, it returns the complete, copyable output right in the chat instead
  of a truncated "… omitted N lines" diff.

### Also

- **Voice key retention** — the OpenAI voice key is remembered across restarts
  and new browser tabs, so the voice line stops asking you to set it up again.
