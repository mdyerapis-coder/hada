...
# ---------------------------------------------------------------------------
# Configuration — locked v3 values
# ---------------------------------------------------------------------------
...
# Hardcoded locked v3 candidate SHA-256 (must match deploy-v3/HADA-M1-gcp-candidate-v3.zip).
...
EXPECTED_CANDIDATE_SHA256_V3="7d969ee44874837a584dcd3363dd4c72c0816fc46d3054300a896d0a37686204"

CANDIDATE_ARCHIVE="${HADA_PHASE_B0_CANDIDATE_ARCHIVE:-${DEPLOY_DIR}/deploy-v3/HADA-M1-gcp-candidate-v3.zip}"
CANDIDATE_SHA256="${HADA_PHASE_B0_CANDIDATE_SHA256_FILE:-${DEPLOY_DIR}/deploy-v3/HADA-M1-gcp-candidate-v3.zip.sha256}"

# Versioned, checksum-locked v3 manifest (never the evidence/phase-b0 path).
# Test-lib mode may override RUN_DIR/MANIFEST_FILE/CANDIDATE_* before sourcing.
if [[ -z "${HADA_PHASE_B0_TEST_LIB:-}" ]]; then
    RUN_DIR="${EVIDENCE_DIR}/preflight-run-${TIMESTAMP}"
fi
MANIFEST_FILE="${MANIFEST_FILE:-${DEPLOY_DIR}/deploy-v3/candidate-manifest-v3.txt}"
...
b0_verify_candidate_checksum() {
    ...
    sha_file="$(awk 'NR==1{print $1}' "${CANDIDATE_SHA256}")"
    {
        ...
        echo "expected: ${EXPECTED_CANDIDATE_SHA256_V3}"
    } >> "${rd}/archive-checksum-verify.txt" 2>/dev/null || true
    [[ "${actual}" == "${EXPECTED_CANDIDATE_SHA256_V3}" ]] || return 1
    [[ "${sha_file}" == "${EXPECTED_CANDIDATE_SHA256_V3}" ]] || return 1
    return 0
}
...
if ! b0_verify_candidate_checksum "${RUN_DIR}"; then
    echo "FAIL: candidate archive checksum verification failed (hash mismatch vs locked v3)"
    EXIT_STATUS=1
    exit 1
fi
echo "PASS: candidate archive checksum verified (matches locked v3 hash)"

# Bind evidence: candidate-sha256.txt (exactly the locked v3 hash) and
...
printf '%s\n' "${EXPECTED_CANDIDATE_SHA256_V3}" > "${RUN_DIR}/candidate-sha256.txt"
{
...
# versioned, checksum-locked v3 manifest. The manifest lives at
# deploy-v3/candidate-manifest-v3.txt (NOT evidence/phase-b0/...). A missing
# or altered manifest (including a stale v1 manifest) fails BEFORE remote
# contact via the sha256sum -c below.
echo ""
echo "  Verifying candidate manifest from extracted root (versioned v3 manifest)..."
...
if [[ "${FINAL_PASS}" -ne 1 ]]; then
  ...
  # Bind evidence: preflight-summary.txt records PASS for every required v3
  # check. This file is consumed by the Phase B Gate 0f evidence lock.
  {
    ...
    echo "candidate-sha256: ${EXPECTED_CANDIDATE_SHA256_V3}"
    echo "project: ${PROJECT}"
    echo "zone: ${ZONE}"
    echo "vm: ${VM}"
