...
# ---------------------------------------------------------------------------
# Configuration — locked v4 values
# ---------------------------------------------------------------------------
...
# Hardcoded locked v4 candidate SHA-256.
# Verification does NOT rely on the path-dependent `sha256sum -c` of the .sha256
# file; both the computed archive hash and the first field of the .sha256 file
# are compared against this exact value.
EXPECTED_CANDIDATE_SHA256_V4="d5582879cba20d92881ba013c68c4b9df3f9e36a3d0ce22aaad0a53bd33856ac"

CANDIDATE_ARCHIVE="${HADA_PHASE_B0_CANDIDATE_ARCHIVE:-${DEPLOY_DIR}/deploy-v4/HADA-M1-gcp-candidate-v4.zip}"
CANDIDATE_SHA256="${HADA_PHASE_B0_CANDIDATE_SHA256_FILE:-${DEPLOY_DIR}/deploy-v4/HADA-M1-gcp-candidate-v4.zip.sha256}"

# Versioned, checksum-locked v4 manifest (never the evidence/phase-b0 path).
# Test-lib mode may override RUN_DIR/MANIFEST_FILE/CANDIDATE_* before sourcing.
if [[ -z "${HADA_PHASE_B0_TEST_LIB:-}" ]]; then
    RUN_DIR="${EVIDENCE_DIR}/preflight-run-${TIMESTAMP}"
fi
MANIFEST_FILE="${MANIFEST_FILE:-${DEPLOY_DIR}/deploy-v4/candidate-manifest-v4.txt}"

# ---------------------------------------------------------------------------
# Test-library hooks (no SSH, no side effects beyond RUN_DIR writes)
# ---------------------------------------------------------------------------
# Callable checksums/manifest verification, shared by the real run and tests.
b0_verify_candidate_checksum() {
    # $1 = run dir (for evidence text). Returns 0 on match, 1 on mismatch.
    local rd="${1:-${RUN_DIR}}"
    local actual sha_file
    actual="$(sha256sum "${CANDIDATE_ARCHIVE}" | awk '{print $1}')"
    sha_file="$(awk 'NR==1{print $1}' "${CANDIDATE_SHA256}")"
    {
        echo "actual:   ${actual}"
        echo "sha-file: ${sha_file}"
        echo "expected: ${EXPECTED_CANDIDATE_SHA256_V4}"
    } >> "${rd}/archive-checksum-verify.txt" 2>/dev/null || true
    [[ "${actual}" == "${EXPECTED_CANDIDATE_SHA256_V4}" ]] || return 1
    [[ "${sha_file}" == "${EXPECTED_CANDIDATE_SHA256_V4}" ]] || return 1
    return 0
}

b0_verify_candidate_manifest() {
    # $1 = extracted candidate root; $2 = run dir. Returns 0 on match, 1 otherwise.
    local root="$1" rd="${2:-${RUN_DIR}}"
    ( cd "${root}" && sha256sum -c "${MANIFEST_FILE}" ) > "${rd}/manifest-verify.txt" 2>&1
    return $?
}

# When sourced as a test library, expose config + functions and stop here.
if [[ -n "${HADA_PHASE_B0_TEST_LIB:-}" ]]; then
...
REMOTE_CONTACTED=0
AFTER_CAPTURE_ATTEMPTED=0
AFTER_CAPTURE_SUCCEEDED=0
CONTAINER_STATE_CHANGED="UNKNOWN"
IMAGE_STATE_CHANGED="UNKNOWN"
REMOTE_CLEANUP_RESULT="NOT_REQUIRED"
FAILED_GATE="none"
OVERALL_RESULT="FAIL"
...
capture_after_state() {
  AFTER_CAPTURE_ATTEMPTED=1
  local ps_rc=0 images_rc=0
  set +e
  ${SSH_CMD} --command='raw="$(sudo -n docker ps -aq)" || { rc=$?; echo "FAIL: sudo -n docker ps -aq failed with rc=${rc}" >&2; exit "${rc}"; }; if [[ -n "${raw}" ]]; then printf "%s\n" "${raw}" | sort; else printf "NONE\n"; fi' \
    2>>"${RUN_DIR}/ssh-stderr.log" >"${RUN_DIR}/docker-ps-after.txt"
  ps_rc=$?
  printf '%s\n' "${ps_rc}" >"${RUN_DIR}/docker-ps-after.rc"
  ${SSH_CMD} --command='raw="$(sudo -n docker images -q)" || { rc=$?; echo "FAIL: sudo -n docker images -q failed with rc=${rc}" >&2; exit "${rc}"; }; if [[ -n "${raw}" ]]; then printf "%s\n" "${raw}" | sort -u; else printf "NONE\n"; fi' \
    2>>"${RUN_DIR}/ssh-stderr.log" >"${RUN_DIR}/docker-images-after.txt"
  images_rc=$?
  printf '%s\n' "${images_rc}" >"${RUN_DIR}/docker-images-after.rc"
  set -e

  if (( ps_rc == 0 && images_rc == 0 )); then
    AFTER_CAPTURE_SUCCEEDED=1
  fi
  if (( ps_rc == 0 )) && [[ -f "${RUN_DIR}/docker-ps-before.txt" ]]; then
    if diff "${RUN_DIR}/docker-ps-before.txt" "${RUN_DIR}/docker-ps-after.txt" >"${RUN_DIR}/container-diff.txt" 2>&1; then
      CONTAINER_STATE_CHANGED="NO"
      echo "PASS: container list unchanged" >>"${RUN_DIR}/container-diff.txt"
    else
      CONTAINER_STATE_CHANGED="YES"
    fi
  else
    printf 'UNKNOWN: before or after container capture unavailable\n' >"${RUN_DIR}/container-diff.txt"
  fi
  if (( images_rc == 0 )) && [[ -f "${RUN_DIR}/docker-images-before.txt" ]]; then
    if diff "${RUN_DIR}/docker-images-before.txt" "${RUN_DIR}/docker-images-after.txt" >"${RUN_DIR}/image-diff.txt" 2>&1; then
      IMAGE_STATE_CHANGED="NO"
      echo "PASS: image list unchanged" >>"${RUN_DIR}/image-diff.txt"
    else
      IMAGE_STATE_CHANGED="YES"
    fi
  else
    printf 'UNKNOWN: before or after image capture unavailable\n' >"${RUN_DIR}/image-diff.txt"
  fi
  {
    echo "=== Container diff ==="; cat "${RUN_DIR}/container-diff.txt"
    echo; echo "=== Image diff ==="; cat "${RUN_DIR}/image-diff.txt"
  } >"${RUN_DIR}/before-after-diff.txt"
  if [[ "${CONTAINER_STATE_CHANGED}" == "NO" && "${IMAGE_STATE_CHANGED}" == "NO" ]]; then
    echo "PASS: Docker state unchanged during preflight" >"${RUN_DIR}/state-check.txt"
  else
    echo "FAIL: Docker state comparison incomplete or changed" >"${RUN_DIR}/state-check.txt"
  fi
}
...
write_preflight_summary() {
  {
    ...
    echo "candidate-sha256: ${EXPECTED_CANDIDATE_SHA256_V4}"
    echo "project: ${PROJECT}"
    echo "zone: ${ZONE}"
    echo "vm: ${VM}"
    echo "after-state-capture-succeeded: $([[ ${AFTER_CAPTURE_SUCCEEDED} -eq 1 ]] && echo YES || echo NO)"
    echo "container-state-changed: ${CONTAINER_STATE_CHANGED}"
    echo "image-state-changed: ${IMAGE_STATE_CHANGED}"
    echo "remote-cleanup-result: ${REMOTE_CLEANUP_RESULT}"
    if [[ "${OVERALL_RESULT}" == "PASS" ]]; then
      echo "candidate-checksum: PASS"; echo "candidate-manifest: PASS"
      echo "compose-version: PASS"; echo "compose-render: PASS"
      echo "port-assertion: PASS"; echo "volume-assertion: PASS"
      echo "container-state-unchanged: PASS"; echo "image-state-unchanged: PASS"
    fi
  } >"${RUN_DIR}/preflight-summary.txt"
}

cleanup() {
  local rc=$? cleanup_rc=0 final_rc
  ...
  final_rc=${EXIT_STATUS}
  (( final_rc == 0 )) && final_rc=${rc}
  if [[ "${REMOTE_CONTACTED}" == "1" && "${AFTER_CAPTURE_ATTEMPTED}" == "0" ]]; then
...
if [[ -z \"\${DIR}\" ]]; then
  ...
  rm -rf \"\${DIR}\"
  echo \"removed: \${DIR}\"
fi" 2>>"${RUN_DIR}/cleanup-stderr.log"
      cleanup_rc=$?
      set -e
      if (( cleanup_rc == 0 )); then REMOTE_CLEANUP_RESULT="PASS"; else REMOTE_CLEANUP_RESULT="FAIL(rc=${cleanup_rc})"; final_rc=1; fi
      echo "[cleanup] remote cleanup attempted: ${REMOTE_DIR}"
    fi
  fi

  if [[ "${PREFLIGHT_COMPLETED}" == "1" && "${AFTER_CAPTURE_SUCCEEDED}" == "1" && "${CONTAINER_STATE_CHANGED}" == "NO" && "${IMAGE_STATE_CHANGED}" == "NO" && "${REMOTE_CLEANUP_RESULT}" != FAIL* ]]; then
    OVERALL_RESULT="PASS"
    FAILED_GATE="none"
  else
    OVERALL_RESULT="FAIL"
...
# Step 0: Verify the candidate archive checksum locally (direct hash compare).
# Delegates to b0_verify_candidate_checksum (shared with tests). Any mismatch
# fails BEFORE any remote contact.
if ! b0_verify_candidate_checksum "${RUN_DIR}"; then
    echo "FAIL: candidate archive checksum verification failed (hash mismatch vs locked v4)"
    EXIT_STATUS=1
    exit 1
fi
echo "PASS: candidate archive checksum verified (matches locked v4 hash)"

# Bind evidence: candidate-sha256.txt (exactly the locked v4 hash) and
# target-identity.txt (project/zone/vm). Written locally, before any remote
# contact, so the evidence is cryptographically bound to this candidate.
printf '%s\n' "${EXPECTED_CANDIDATE_SHA256_V4}" > "${RUN_DIR}/candidate-sha256.txt"
{
    echo "project=${PROJECT}"
    echo "zone=${ZONE}"
    echo "vm=${VM}"
} > "${RUN_DIR}/target-identity.txt"
echo "  -> wrote candidate-sha256.txt and target-identity.txt"
...
# Verify the candidate manifest from the extracted candidate root using the
# versioned, checksum-locked v4 manifest. The manifest lives at
# deploy-v4/candidate-manifest-v4.txt (NOT evidence/phase-b0/...). A missing
# or altered manifest (including a stale v1 manifest) fails BEFORE remote
# contact via the sha256sum -c below.
echo ""
echo "  Verifying candidate manifest from extracted root (versioned v4 manifest)..."
...
b0_verify_candidate_manifest "${CANDIDATE_ROOT}" "${RUN_DIR}"
MANIFEST_RC=$?
...
echo "PASS: candidate manifest verified from extracted root (${MANIFEST_FILE})"
...
FAILED_GATE="before-state-capture"
...
FAILED_GATE="remote-temp-create"
...
FAILED_GATE="preflight-file-upload"
...
if [[ "${VOL_RC}" -ne 0 ]]; then
  ...
  [[ "${PORT_RC}" -eq 0 ]] && FAILED_GATE="volume-assertion"
else
...
if [[ "${PORT_RC}" -eq 0 && "${VOL_RC}" -eq 0 ]]; then FAILED_GATE="after-state-capture"; fi
capture_after_state
if [[ "${AFTER_CAPTURE_SUCCEEDED}" != "1" || "${CONTAINER_STATE_CHANGED}" != "NO" || "${IMAGE_STATE_CHANGED}" != "NO" ]]; then
  echo "FAIL: AFTER-state capture or comparison failed"
  EXIT_STATUS=1
else
  echo "PASS: Docker state unchanged during preflight"
fi
